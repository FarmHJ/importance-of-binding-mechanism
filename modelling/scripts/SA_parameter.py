...
for drug in drug_list:
    ...
    filename2 = 'SA_' + drug + '_' + parameter_interest + '_check.csv'
    if os.path.exists(saved_data_dir + filename):
        os.remove(saved_data_dir + filename)
    if os.path.exists(saved_data_dir + filename2):
        os.remove(saved_data_dir + filename2)
    ...
    for num, param in enumerate(param_range):
        ...
        Hill_curve_coefs, drug_conc_Hill, peaks_norm = \
            ComparisonController.compute_Hill(drug_model,
                                              Hill_upper_thres=0.9,
                                              Hill_lower_thres=0.1)
    ...
    ####################
        ...
        Hill_curve_coefs, drug_conc_Hill, peaks_norm = \
            ComparisonController.compute_Hill(drug_model,
                                              drug_conc=drug_conc_Hill,
                                            #   drug_conc=drug_conc_Hill_norm,
                                              Hill_upper_thres=0.7,
                                              Hill_lower_thres=0.3)
        ...
        # plt.xscale('log')
        # plt.savefig(saved_fig_dir + str(param) + '_test_peak.pdf')
        # plt.close()

        drug_conc_AP_norm = [i / (np.power(param, Hill_n)) for i in drug_conc_AP]
        try:
            APD_trapping, APD_conductance = ComparisonController.APD_sim(
                AP_model, Hill_curve_coefs, drug_conc=drug_conc_AP_norm)
        ...
        index_dict = {'drug_conc_Hill': conc_Hill_ind,
                        ...
                        'APD_conductance': conc_AP_ind, 'RMSE': ['RMSE'],
                        'MAE': ['MAE']}
        all_index = [(i, j) for i in index_dict.keys() for j in
                        index_dict[i]]
        index = pd.MultiIndex.from_tuples(all_index)

        big_df = pd.DataFrame(
            drug_conc_Hill + list(peaks_norm) + list(Hill_curve_coefs) +
            list(param_values.values[0]) + list(drug_conc_AP_norm) +
            APD_trapping + APD_conductance + [RMSError] + [MAError],
            index=index)

        if num != 0:
            previous_df = pd.read_csv(saved_data_dir + filename2,
                                        header=[0, 1], index_col=[0],
                                        skipinitialspace=True)
            comb_df = pd.concat([previous_df, big_df.T])
        else:
        ...
        comb_df.to_csv(saved_data_dir + filename2)
