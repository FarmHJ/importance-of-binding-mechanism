# Calibrate the conductance reduction model from synthetic experimental data (CiPA hERG model) and compare the APD90 at steady state and transient phase.
# Output:
...
# 4. hERG current at various drug concentration simulated from conductance calibrated hERG model.
# 5. Comparison of peak hERG current between both models, i.e. CiPA hERG model and conductance hERG model.
...
# 9. 10 pulses of action potentials simulated from both models (transient phase).
...
if drug == 'dofetilide':
    drug_conc = [0, 0.1, 1, 30, 100, 300, 500, 1000] #nM #dofetilide
elif drug == 'verapamil':
    drug_conc = [0, 0.1, 1, 30, 300, 500, 1000, 10000, 1e5] #nM #verapamil
repeats = 1000
drug_labels = [str(i) + ' nM' for i in drug_conc]

saved_data_dir = '../../examples/simulation_data/binding_kinetics_comparison/' + drug + '/' + protocol_name + '/'
result_filename = 'OHaraCiPA-conductance-fit.txt'

testing_fig_dir = '../../figures/testing/'
final_fig_dir = '../../figures/binding_kinetics_comparison/' + drug + '/' + protocol_name + '/'
...
if plot_fig:
    ...
    plt.tight_layout()
    plt.legend()
    plt.savefig(saved_fig_dir + "fitted_peak_hERG_trapping_" + drug + "_concs.pdf")
...
conductance_scale_df = pd.DataFrame(np.transpose(conductance_scale_est), columns=[drug])
...
for i in range(len(drug_conc)):
    ...
    d2 = drug_model.conductance_simulation(conductance * scale, repeats) #, current_head='ikr')
...
#     plt.savefig(saved_fig_dir + "hERG_conductance_" + drug + "_concs.pdf")
    ...
    fig = modelling.new_figures.FigureStructure(figsize=(5, 4), gridspec=(3, 1))
    ...
    plot.add_multiple(fig.axs[1][0], log_trapping, 'ikr.IKr', labels=labels, color=cmap)
    plot.add_multiple(fig.axs[2][0], log_conductance, 'ikr.IKr', labels=labels, color=cmap)
    
    fig.axs[2][0].legend()
    fig.sharex(['Time (s)'], [(0, pulse_time)])
    fig.sharey(['Voltage (mV)', 'Current (A/F)\ntrapped model', 'Current (A/F)\nnontrapped model'])
...
if plot_fig:
    row, col = 3, 3
    fig = modelling.figures.CurrentPlot(drug_model)
    fig.hERG_compare(log_trapping, log_conductance, drug_conc, pulse_time, row=row, col=col)
...
if plot_fig:
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(saved_fig_dir + "norm_peak_compare_hERG_" + drug + "_concs.pdf")
...
if plot_fig:
    if steady_state:
        ...
        fig.add_plot_2AP_various(AP_trapping, drug_conc, plotting_pulse_time, 2)
        fig.axs[0].set_title("O'Hara-CiPA model (with trapping)")
        plt.savefig(saved_fig_dir + "2AP_hERG_trapping_" + drug + "_concs.pdf")
        plt.close()
        
        fig = modelling.figures.CurrentPlot(AP_model)
        fig.add_plot_2AP_various(AP_conductance, drug_conc, plotting_pulse_time, 2)
        fig.axs[0].set_title("O'Hara model (without trapping)")
        plt.savefig(saved_fig_dir + "2AP_hERG_conductance_" + drug + "_concs.pdf")
        plt.close()
       
        fig = modelling.new_figures.FigureStructure(figsize=(8, 4), gridspec=(3, 2), wspace=0.08)
        plot = modelling.new_figures.FigurePlot()
        cmap = matplotlib.cm.get_cmap('viridis')

        plot.add_continuous(fig.axs[0][0], AP_trapping[0], 'stimulus.i_stim')
        plot.add_multiple_continuous(fig.axs[1][0], AP_trapping, 'membrane.V', cmap=cmap, labels=drug_labels)
        plot.add_multiple_continuous(fig.axs[2][0], AP_trapping, 'ikr.IKr', cmap=cmap, labels=drug_labels)
        plot.add_continuous(fig.axs[0][1], AP_conductance[0], 'stimulus.i_stim')
        plot.add_multiple_continuous(fig.axs[1][1], AP_conductance, 'membrane.V', cmap=cmap, labels=drug_labels)
        plot.add_multiple_continuous(fig.axs[2][1], AP_conductance, 'ikr.IKr', cmap=cmap, labels=drug_labels)
      
        unique = fig.legend_without_duplicate_labels(fig.axs[2][1])
        fig.axs[2][1].legend(*zip(*unique), loc='right', bbox_to_anchor=(1.4, 0.6))
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(saved_fig_dir + "APD90_compare_2pulses_hERG_" + drug + "_concs.pdf")
        plt.close()
    ...
    #     hERG_reduc_conduct = []
        ...
        for i in range(len(drug_conc)):
            ax.plot(np.arange(save_signal), np.array(hERG_peak_trapping[i]), 'o-',
    ...
    #         hERG_reduc_trap.append((hERG_peak_conc_trap[0] - hERG_peak_conc_trap[-1]) / hERG_peak_conc_trap[0])
    #     
    #         hERG_peak_conc_conduct = hERG_peak_conductance[i]
    #         hERG_reduc_conduct.append((hERG_peak_conc_conduct[0] - hERG_peak_conc_conduct[-1]) / hERG_peak_conc_conduct[0])
    ...
    #     plt.savefig(saved_fig_dir + 'peak_reduction.pdf')
        ...
        fig = modelling.new_figures.FigureStructure(figsize=(10, 4), gridspec=(2, 1), height_ratios=[1, 1])
        plot = modelling.new_figures.FigurePlot()
        
        plot.add_multiple_continuous(fig.axs[0][0], AP_trapping, 'membrane.V', cmap=cmap) 
        plot.add_multiple_continuous(fig.axs[1][0], AP_trapping, 'ikr.IKr', labels=labels, cmap=cmap) 
        ...
        fig.savefig(saved_fig_dir + "10AP_transient_hERG_trapping_" + drug + "_concs.pdf")
        plt.close()
        ...
        plot.add_multiple_continuous(fig.axs[0][0], AP_conductance, 'membrane.V', cmap=cmap) 
        plot.add_multiple_continuous(fig.axs[1][0], AP_conductance, 'ikr.IKr', labels=labels, cmap=cmap) 
        ...
        fig.savefig(saved_fig_dir + "10AP_transient_hERG_conductance_" + drug + "_concs.pdf")
        plt.close('all')
...
#                 ax.scatter(AP_conductance[i]['membrane.V', pulse], AP_conductance[i]['ikr.IKr', pulse],
#                         color=cmap(norm(pulse)), zorder=-10)
#             ax.set_rasterization_zorder(0)
#             plt.savefig(saved_fig_dir + "phase_plane_" + str(drug_conc[i]) + ".pdf")
        ...
        plt.xlabel('Sweeps')
        plt.ylabel(r'APD$_{90}$')
        plt.savefig(saved_fig_dir + "10APD90_compare_hERG_" + drug + "_concs.pdf",
                bbox_inches='tight')
        plt.close()
        
        fig = modelling.new_figures.FigureStructure(figsize=(10, 4), gridspec=(2, 4),
                height_ratios=[1, 1], hspace=0.2, wspace=0.08)
        for i in range(len(drug_conc)):
            for j in range(len(drug_conc)):
                if i == j:
                    fig.axs[int(i / 4)][i % 4].plot(np.arange(save_signal), np.array(APD_trapping[j]), 'o-',
                        label='with trapping', color='orange', zorder=5)
                    fig.axs[int(i / 4)][i % 4].plot(np.arange(save_signal), np.array(APD_conductance[j]), '^--',
        ...
        lgds = [(h, l) for i, (h, l) in enumerate(zip(handles, labels)) if i <= 1]
        ...
        # Plot APD90 of 10 pulses - adjustments for specific case (verapamil, Milnes)
