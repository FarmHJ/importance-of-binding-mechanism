...
# Left panel
fig.axs[0][0].plot(np.log(drug_conc[1:]), APD_trapping[1:],
                   'o', color='orange', label='state dependent drug block')
fig.axs[0][0].plot(np.log(drug_conc[1:]), APD_conductance[1:],
                   '^', color='blue', label='conductance scaling', alpha=0.8)
fig.axs[0][0].plot(np.log(drug_conc[1:]), EAD_marker, 'o', color='k',
                   marker=(5, 2), label='EAD-like AP')
fig.axs[0][0].set_xlabel('Drug concentration (log)')
fig.axs[0][0].set_ylabel(r'APD$_{90}$')
...
# Right panel
fig.axs[0][1].plot(np.log(drug_conc[1:]), APD_trapping[1:],
                   'o', color='orange', label='state dependent drug block')
fig.axs[0][1].plot(np.log(drug_conc[1:]), APD_conductance[1:],
                   '^', color='blue', label='conductance scaling', alpha=0.8)
fig.axs[0][1].plot(np.log(drug_conc[1:]), EAD_marker, 'o', color='k',
                   marker=(5, 2), label='EAD-like AP')
fig.axs[0][1].set_xlabel('Drug concentration (log)')
fig.axs[0][1].set_ylabel(r'APD$_{90}$')
