...
if not fig_short:
    ...
    plot.add_multiple(panel1[1][0], CiPA_hERG_log, 'ikr.IKr', labels=labels,
                      color=cmap)
    plot.add_single(panel2[0][0], conductance_hERG_log[0],
                    'membrane.V', color='k')
    plot.add_multiple(panel2[1][0], conductance_hERG_log, 'ikr.IKr',
    ...
    APD_conductance = [max(APD_conductance.loc[i].values.tolist()[(
        1 + 998):-1]) for i in range(APD_conductance.shape[0])]
